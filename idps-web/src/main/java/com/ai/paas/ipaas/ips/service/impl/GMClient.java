...
import com.ai.paas.GeneralException;
...
public class GMClient {
    ...
    public GMClient(String parameter, AuthDescriptor ad) {
        try {
            ...
            if (paras != null) {
                ...
                config.setTestOnGet(paras.get(TEST_ON_BORROW_KEY).getAsBoolean());
                config.setTestOnReturn(paras.get(TEST_ON_RETURN_KEY).getAsBoolean());
    ...
    public String getSourceImagePath(String imageName, String imageType) throws Exception {
        return (imageSrcRoot + (imageSrcRoot.endsWith(File.separator) ? "" : File.separator) + getFirstPath(imageName)
                + File.separator + getSecondPath(imageName) + File.separator + imageName + imageType);
    }
    ...
    public String scaleImage(String uri, String imageName, int type, String imageSize, String imageType,
        ...
        try {
            String command = getCommand(imageName, type, imageSize, targetPath, imageType, isExtent, quality);
            log.debug("{}----GraphicsImage----scaleImage---------command:{}", uri, command);
    ...
    public boolean judgeSize(String srcImage, int minWidth, int minHeight) {
        ...
        try {
            ...
            log.debug("----GraphicsImage----judge---------width:{},height:{}", width, height);
    ...
    private String getConvertTpyrCommand(String src, String desc) {
        ...
        if (desc.lastIndexOf(".jpg") >= 0) {
            ...
            // 按png处理
            return " convert" + " -quality 100 " + " " + src + " +dither -depth 8 " + desc;
        }
    }
